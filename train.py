...
transfers = transforms.Compose([
        ...
        transforms.ToTensor(),
        #transforms.Normalize(mean=[0.022, 0.02, 0.021], std=[0.136, 0.128, 0.133])  # slide_gbm
        transforms.Normalize(mean=[0.04, 0.037, 0.039], std=[0.179, 0.169, 0.176])  # slide_lusc
    ])

train0 = Loader.PatchData.split_cluster('./data/TCGA-LUSC-slide.csv', 'Train', transfer=transfers)  # slide
#valid0 = Loader.PatchData.split_cluster('./data/TCGA-LUSC-slide.csv', 'Valid', transfer=transfers)  # inpainting
...
for epoch in range(epoch_size):
    ...
    for iteration, data in loop:
        img, img1, img2, img3, label = data  #, img4, img5, img6, img7, img8, img9, img0
        ...
        with torch.no_grad():
            feature = pre_model_slide(img)  # [b, 1024, 16, 16]
        ...
        pred = model(feature, feature_patch)
        loss1, n_observed = criterion(pred, label)
        loss2 = concordance_index(pred, label)
        loss = loss1 + loss2
    ...
    writer.add_scalar("Train_loss", loss_board/i, epoch)
...
for iteration, data in enumerate(dataloader_var):
    ...
    img_var, img1, img2, img3, img4, img5, img6, img7, img8, img9, img0, label_var = data  #
